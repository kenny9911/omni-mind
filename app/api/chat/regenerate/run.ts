...
export async function regenerateStream(
  ...
  // Replace in place: drop old assistant message (seq=1) + this turn's usage.
  await ctx.db
    .delete(messages)
    .where(and(eq(messages.turnId, input.turnId), eq(messages.seq, 1)));
  await ctx.db.delete(usageRecords).where(eq(usageRecords.turnId, input.turnId));

  // Reset the turn to streaming for the fresh run.
  await ctx.db
    .update(turns)
